fn build() {
    xshell::cmd!("wasm-pack build  website --target web --out-dir ../out").read().unwrap();
...
fn serve() {
    ...
    xshell::cmd!("python3 -m http.server --directory out/").read().unwrap();
}

fn github_setup() {
    xshell::cmd!("rustup target add wasm32-unknown-unknown").read().unwrap();
}
